...
from monitoring.performance_tracker import Trade, get_performance_tracker
...
class LearningEngine:
    ...
    def calculate_optimal_weights(
        ...
        for source, perf in performances.items():
            ...
            new_weight = (
                current_weight + (target_weight - current_weight) * self.learning_rate
            )
    ...
    async def optimize_weights(self) -> dict[str, float]:
        ...
        for source, new_weight in new_weights.items():
            ...
            logger.info(
                f"  {source}: {old_weight:.3f} → {new_weight:.3f} ({change:+.3f})"
            )
