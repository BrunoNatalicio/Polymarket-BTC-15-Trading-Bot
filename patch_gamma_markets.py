...
def apply_gamma_markets_patch():
    ...
    try:
        ...
        logger.info(
            "✓ Patched gamma_markets.build_markets_query (array parameter handling)"
        )
        ...
        async def _load_all_using_gamma_markets(
            ...
            try:
                ...
                for market in markets:
                    try:
                        normalized_market = (
                            gamma_markets.normalize_gamma_market_to_clob_format(market)
                        )
                        ...
                        self._log.error(
                            f"Error processing market {market.get('slug', 'unknown')}: {e}"
                        )
                        continue

                self._log.info(
                    f"Successfully loaded {loaded_count} instruments from {len(markets)} markets"
                )

                if btc_count > 0:
                    self._log.info("✓ BTC markets found and loaded!")
                else:
