...
class GrafanaMetricsExporter:
    ...
    def _setup_metrics(self) -> None:
        ...
        self.max_drawdown = Gauge(
            "trading_max_drawdown", "Maximum drawdown as percentage"
        )

        # Trade counters
        self.total_trades = Counter("trades_total", "Total number of trades executed")

        self.winning_trades = Counter(
            "trading_winning_trades", "Number of winning trades"
        )
        ...
        self.open_positions = Gauge(
            "trading_open_positions", "Number of currently open positions"
        )
        ...
        self.risk_utilization = Gauge(
            "trading_risk_utilization", "Percentage of risk limits utilized"
        )

        self.current_capital = Gauge(
            "trading_current_capital", "Current account capital in USD"
        )

        # Signal metrics
        self.avg_signal_score = Gauge(
            "trading_avg_signal_score", "Average signal score (0-100)"
        )

        self.avg_signal_confidence = Gauge(
            "trading_avg_signal_confidence", "Average signal confidence (0-1)"
        )
        ...
        # Order metrics
        self.orders_placed = Counter("trading_orders_placed", "Total orders placed")

        self.orders_filled = Counter("trading_orders_filled", "Total orders filled")
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            logger.info("  Supports: GET, POST, OPTIONS")
