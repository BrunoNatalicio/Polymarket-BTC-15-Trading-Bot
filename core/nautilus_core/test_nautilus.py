...
async def test_instruments():
    ...
    try:
        ...
        # Check instruments
        console.print("Checking registered instruments...", end="")

        all_instruments = registry.get_all()

        if len(all_instruments) >= 3:
            console.print(
                f" [green]✓ {len(all_instruments)} instruments registered[/green]"
            )
        else:
...
async def test_event_dispatcher():
    ...
    try:
        ...
        dispatcher.dispatch_price_update(
            source="test", price=65000.0, metadata={"test": True}
        )
...
async def test_data_engine():
    ...
    try:
        ...
        if status["instruments_registered"] >= 3:
            console.print(
                f" [green]✓ {status['instruments_registered']} instruments[/green]"
            )
        else:
