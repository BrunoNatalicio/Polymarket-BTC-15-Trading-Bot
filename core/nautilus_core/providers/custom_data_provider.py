...
from core.ingestion.adapters.unified_adapter import MarketData, UnifiedDataAdapter
...
class CustomDataProvider:
    ...
    def __init__(
        ...
        # Our unified adapter
        self.adapter: UnifiedDataAdapter | None = None
    ...
    async def _on_sentiment_update(self, data) -> None:
        ...
        loguru_logger.debug(
            f"Sentiment update: {data.score}/100 - {data.classification}"
        )

    def _get_instrument_id(self, source: str) -> InstrumentId | None:
        """
        ...
        try:
            ...
            if data.bid and data.ask:
                ...
                bid_price = Price.from_str(
                    bid_str if "." in bid_str else f"{bid_str}.0"
                )
                ask_price = Price.from_str(
                    ask_str if "." in ask_str else f"{ask_str}.0"
                )
            else:
    ...
    def get_latest_price(self, source: str) -> Decimal | None:
        """Get latest price from a source."""
        return self._last_prices.get(source)

    def get_price_consensus(self) -> dict | None:
