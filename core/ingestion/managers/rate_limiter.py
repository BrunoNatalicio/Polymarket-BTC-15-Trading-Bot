...
class RateLimiter:
    ...
    async def acquire(self, wait: bool = True) -> bool:
        ...
        async with self._lock:
            ...
            wait_time = (
                oldest_request + timedelta(seconds=self.time_window) - now
            ).total_seconds()
    ...
    def get_remaining(self) -> int:
        ...
        current_requests = sum(
            1 for req_time in self._requests if req_time >= cutoff_time
        )
...
class MultiSourceRateLimiter:
    ...
    def add_limiter(
        self, source: str, max_requests: int, time_window: int = 60
    ) -> RateLimiter:
        """
    ...
    def get_stats(self, source: str | None = None) -> dict[str, any]:
        ...
        if source:
            ...
            return {
                name: limiter.get_stats() for name, limiter in self.limiters.items()
            }
...
# Singleton instance
_rate_limiter_instance: MultiSourceRateLimiter | None = None
