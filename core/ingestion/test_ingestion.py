...
async def test_unified_adapter():
    ...
    try:
        ...
        if consensus:
            ...
            console.print(
                f"  Price spread: ${consensus['spread']:,.2f} ({consensus['spread_percent']:.2f}%)"
            )
...
async def test_data_validator():
    ...
    try:
        ...
        if anomaly:
            ...
            console.print(
                " [yellow]⚠ No anomaly detected (may need more data)[/yellow]"
            )
...
async def test_rate_limiter():
    ...
    try:
        ...
        for source, source_stats in stats.items():
            console.print(
                f"  [{source}] {source_stats['current_requests']}/{source_stats['max_requests']} "
