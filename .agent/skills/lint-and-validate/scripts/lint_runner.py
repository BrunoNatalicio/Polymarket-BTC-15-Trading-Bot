...
def detect_project_type(project_path: Path) -> dict:
    ...
    if (project_path / "pyproject.toml").exists() or (project_path / "requirements.txt").exists():
        ...
        # Check for ruff
        result["linters"].append({"name": "ruff", "cmd": ["uv", "run", "ruff", "check", "."]})

        # Pyright scoped to files known to be type-clean. bot.py and several
        # legacy modules carry ~111 pre-existing pyright errors; gating the
        # whole repo would block every commit. Widen this list as files get
        # cleaned up.
