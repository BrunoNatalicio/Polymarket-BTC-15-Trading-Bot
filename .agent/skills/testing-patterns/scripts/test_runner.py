...
def detect_test_framework(project_path: Path) -> dict:
    ...
    # Python project — this repo has NO pytest suite. Tests are standalone
    # scripts (see .context/docs/testing-strategy.md). Letting pytest
    # auto-collect them would execute scripts that hit real external APIs
    # (Polymarket, exchanges) inside the pre-commit hook. Run only the
    # hermetic gate scripts listed here; extend the list as new hermetic
    # test scripts are added.
    if (project_path / "pyproject.toml").exists() or (project_path / "requirements.txt").exists():
        result["type"] = "python"
        result["framework"] = "standalone scripts"
        result["cmd"] = ["uv", "run", "python", "test_tradingview_webhook.py"]
        result["coverage_cmd"] = None
...
def run_tests(cmd: list, cwd: Path) -> dict:
    ...
    try:
        proc = subprocess.run(
            ...
            encoding='utf-8',
            errors='replace',
            timeout=1140  # 19 min — abaixo do timeout do checklist (1200s) para
            # falhar aqui com mensagem clara em vez de ser morto pelo wrapper
        )
        ...
        result["error"] = "Timeout after 1140s"
...
def main():
    ...
    # If pytest.ini was found in a subdirectory, run tests from there
    pytest_rootdir = test_info.get("pytest_rootdir")
    run_dir = pytest_rootdir if pytest_rootdir else project_path
    ...
    # Run tests from the pytest rootdir (where pytest.ini lives)
    result = run_tests(cmd, run_dir)
