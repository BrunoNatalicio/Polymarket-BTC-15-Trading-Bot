...
# Ensure UTF-8 output on Windows consoles (cp1252 default rejects emoji)
...
CORE_CHECKS = [
    ...
    # Test Runner é BLOQUEANTE (2026-06-10): antes era optional e o gate deixava
    # passar commit com suíte quebrada; o timeout de 300s também derrubava a
    # suíte (~5-6 min sob carga) como falso FAIL→WARN. Ver timeout em run_script.
    ("Test Runner", ".agent/skills/testing-patterns/scripts/test_runner.py", True),
...
def run_script(name: str, script_path: Path, project_path: str, url: Optional[str] = None) -> dict:
    ...
    try:
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            timeout=1200  # 20 min: a suíte completa leva ~5-6 min sob carga de commit
        )
        ...
        if passed:
            ...
            # Diagnóstico (2026-06-10): sem o stdout, uma falha de pytest no hook
            # era indecifrável (stderr vazio). Imprime o tail onde está o resumo.
            if result.stdout:
                tail = result.stdout.strip().splitlines()[-25:]
        ...
        print_error(f"{name}: TIMEOUT (>20 minutes)")
...
def print_summary(results: List[dict]):
    """Print final summary report. Only required failures block the commit."""
    print_header("CHECKLIST SUMMARY")

    passed_count = sum(1 for r in results if r["passed"] and not r.get("skipped"))
    required_failed = sum(1 for r in results if not r["passed"] and not r.get("skipped") and r.get("required", True))
    optional_failed = sum(1 for r in results if not r["passed"] and not r.get("skipped") and not r.get("required", True))
    skipped_count = sum(1 for r in results if r.get("skipped"))

    print(f"Total Checks: {len(results)}")
    print(f"{Colors.GREEN}Passed: {passed_count}{Colors.ENDC}")
    if required_failed:
        print(f"{Colors.RED}Failed (required): {required_failed}{Colors.ENDC}")
    if optional_failed:
        print(f"{Colors.YELLOW}Failed (optional): {optional_failed}{Colors.ENDC}")
    ...
    if required_failed > 0:
        print_error(f"{required_failed} required check(s) FAILED - Please fix before proceeding")
        return False
    elif optional_failed > 0:
        print_warning(f"{optional_failed} optional check(s) have warnings (non-blocking)")
...
Examples:
  ...
  python scripts/checklist.py . --url http://localhost:3000  # Include performance
    ...
    for name, script_path, required in CORE_CHECKS:
        ...
        # If required check fails, stop immediately
