...
class NewsSocialDataSource:
    ...
    def __init__(self):
        ...
        # Cache
        self._last_sentiment: dict[str, Any] | None = None
                ...
                news.append(
                    {
                        ...
                        - article.get("votes", {}).get("negative", 0),
                        "sentiment": "positive"
                        if article.get("votes", {}).get("positive", 0)
                        > article.get("votes", {}).get("negative", 0)
                        else "negative",
                    }
                )
    ...
    async def get_sentiment_score(self) -> float | None:
        """
        ...
        try:
            ...
            if news:
                ...
                positive_count = sum(
                    1 for n in news if n.get("sentiment") == "positive"
                )
    ...
    @property
    def last_sentiment(self) -> dict[str, Any] | None:
...
# Singleton instance
_news_instance: NewsSocialDataSource | None = None
