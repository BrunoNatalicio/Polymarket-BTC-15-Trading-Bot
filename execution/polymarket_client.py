...
class PolymarketClient:
    ...
    def __init__(
        ...
        if not POLYMARKET_AVAILABLE:
            logger.error(
                "Polymarket SDK not available. Install: pip install py-clob-client"
            )
            return
    ...
    async def get_btc_market(self) -> dict[str, Any] | None:
        """
    ...
    async def place_order(
        ...
        try:
            ...
            if response and "orderID" in response:
                ...
                logger.info(
                    f"Order placed: {order_id} {side.upper()} {size} @ {price:.4f}"
                )
    ...
    async def get_positions(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            for token_id, balance in balances.items():
                if token_id != "USDC" and float(balance) > 0:  # noqa: S105
                    positions.append(
                        {
                            "token_id": token_id,
    ...
    async def _get_balance_internal(self) -> dict[str, Decimal] | None:
        ...
        try:
            ...
            return {token: Decimal(str(amount)) for token, amount in balances.items()}
