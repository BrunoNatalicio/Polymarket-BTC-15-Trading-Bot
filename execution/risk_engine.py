...
class RiskEngine:
    ...
    def validate_new_position(
        ...
        if drawdown > self.limits.max_drawdown_pct:
            return (
                False,
                f"Drawdown {drawdown:.1%} exceeds max {self.limits.max_drawdown_pct:.1%}",
            )
    ...
    def calculate_position_size(
        ...
        strength_multiplier = Decimal(str(signal_confidence)) * Decimal(
            str(signal_score / 100)
        )
    ...
    def update_position(
        ...
        if position.stop_loss and self._check_stop_loss(position, current_price):
            self._create_alert(
                "STOP_LOSS", f"Stop loss hit for {position_id}", RiskLevel.HIGH
            )

        if position.take_profit and self._check_take_profit(position, current_price):
            self._create_alert(
                "TAKE_PROFIT", f"Take profit hit for {position_id}", RiskLevel.LOW
            )
    ...
    def remove_position(
        ...
        logger.info(
            f"Closed position: {position_id} P&L: ${realized_pnl:+.2f} ({pnl_pct:+.2%})"
        )
    ...
    def _check_take_profit(
        self, position: PositionRisk, current_price: Decimal
    ) -> bool:
    ...
    def _create_alert(
        self, alert_type: str, message: str, risk_level: RiskLevel
    ) -> None:
        """Create a risk alert."""
        alert = {
