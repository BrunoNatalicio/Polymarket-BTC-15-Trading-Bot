...
class PolymarketBTCIntegration:
    ...
    def __init__(
        ...
        btc_market_condition_id: str | None = None,
    ):
        """
        ...
        self.btc_instrument_id: InstrumentId | None = None
    ...
    async def start(self) -> bool:
        ...
        try:
            ...
            self.node.add_exec_client_factory(
                POLYMARKET, PolymarketLiveExecClientFactory
            )
    ...
    async def place_market_order(
        ...
        if self.simulation_mode:
            logger.info(
                f"[SIMULATION] Would place {side.upper()} order for ${size_usd}"
            )
    ...
    async def place_limit_order(
        ...
        if self.simulation_mode:
            logger.info(
                f"[SIMULATION] Would place {side.upper()} limit @ ${limit_price:.4f}"
            )
            return f"sim_order_{datetime.now().timestamp()}"
        ...
        try:
            ...
            # Calculate token quantity
            token_qty = (
                float(size_usd) / float(limit_price)
                if limit_price > 0
                else float(size_usd) * 2
            )
            precision = instrument.size_precision
            token_qty = round(token_qty, precision)
            ...
            logger.info(
                f"Submitting limit order: {order_side.name} {token_qty:.6f} @ ${limit_price:.4f}"
            )
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "instrument_id": str(self.btc_instrument_id)
            if self.btc_instrument_id
            else None,
...
# Singleton instance
_integration_instance: PolymarketBTCIntegration | None = None
